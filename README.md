...
**离线代币监管定性器** —— 输入一段代币机制描述（白皮书摘要 / 代币经济学 / 问卷），按 **Howey Test 四要素** + **香港 SFC / 新加坡 MAS / 美国 SEC / 欧盟 MiCA 四法域**监管框架，输出分级定性、逐要素证据与风险提示。
...
本工具为求职作品集，非商用产品。诚实性审计发现的全部缺口完整记录于 [`docs/HONESTY_AUDIT.md`](docs/HONESTY_AUDIT.md) —— 共 **16 项**，清单**已冻结**（不再新增清单项与新审计模块）。

**第 1 项（项目级局限）**：**当前无真实案例回归** —— 全部验证输入（含 `demo/` 三个虚构代币 XChain / StoragePay / EcoToken）**均为构造性用例**。构造性验证只能证明「代码在特定输入下行为正确」，**不能**证明「在真实世界 / 真实监管案例下正确」。真实案例回归列为 **Phase 2**。

**状态三分**：

| 状态 | 数量 | 说明 |
|------|------|------|
| ✅ 已修复 | **0 项** | 审计按纪律「只记录不修」，全部缺口原样保留 |
| 🔒 已测试锁定（`expectedFailure`，等修复） | **2 项** | #1 G-A1、#2 问题 4 —— 共 4 个 xFail |
| 📝 仅记录（无测试） | **14 项** | #3–#13（11 项）+ #D1–#D3 数据质量缺口（3 项） |

**修复路线图**：

| 优先级 | 项 | 时间承诺 |
|--------|-----|---------|
| **Phase 1（高）** | #1 G-A1、#2 问题 4、#D2 / #D3（两条 404 死链） | 下个 release 前 |
| **Phase 2（中高）** | #3 G-A3、#4 问题 2、#5 问题 3、#7 C4 | 后续 |
| **Phase 3（设计复核 · 长期）** | #8 C6、#10 C.10、#11 信号分类法适配性、#12 词表语义对齐、#13 utility 披露 | 长期，可能重构 |
| **不承诺修复** | #6 B5（法律争议，补 `dispute_note`）、#9 C7b（补词表）、#D1（弱引用） | 记录即可 |

> 数据质量缺口 #D1–#D3 性质说明：#D2 / #D3 为 **404 死链**（引用不可验证，严重性高）；#D1 为**弱引用**（URL 存活但页面不匹配，可修）。成因是 **SFC 官网迁移未同步**，属引用维护缺失，建议加 CI 定期巡检。

**我们选择公开这些缺口，而非掩盖** —— 合规工具的可信度来自「知道自己哪里不可靠」，而不是「声称自己完美」。**有路线图的 16 项是一个有序 backlog，不是一堆烂摊子。**
