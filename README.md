...
| # | 模式 | 本项目实例 |
|---|------|-----------|
| 1 | 能力缺口 → 包装成「克制」 | 投资资金要素漏了劳务 / 算力 / 流动性对价词，工具报 `unknown` 看似「信息不足」，实为词表没覆盖 |
| 2 | 验证不充分 → 包装成「验证完成」 | 模块 A 初版「5/14 含否定词」的统计：既把不在词表的词算入，又搞错了否定的触发机制——统计与机制双重错误 |
| 3 | 新回归 → 包装成「已知限制」 | B5「锁仓 = weak」被写成法律争议，实为简化未标注 |
| 4 | 修复引入的回归 → 藏在自测盲区 | 问题 4 的子串匹配缺陷在自造用例下不显现，换一个高频词（「消费者权益」）即误判 |
| 5 | 边界数据 / 例子 → 未严格自检 | 演示「否定窗口吞掉命中」的例子若否定词与指标同句，则被否是**正确**行为，不能当 bug 用 |
