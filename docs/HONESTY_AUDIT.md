...
> - **延伸观察（重要）**：本项目目前**不存在任何真实案例回归** —— `demo/` 下 3 个输入亦为**虚构代币**（`demo_security_token.md` = XChain Token、`demo_utility_token.md` = StoragePay Token、`demo_ambiguous_token.md` = EcoToken；名称均为杜撰），而非真实白皮书。故「结果层」的全部证据**均为构造性**。
...
> - **子表 A — 代码 / 设计缺口（#1–#11）**：影响工具**逻辑 / 词表 / 判定 rubric** 的缺口（含需改代码与仅补注释两类），与「港新判定独立性」属于不同维度。
> - **子表 B — 数据质量缺口（#D1–#D3）**：影响**数据可信度 / 引用溯源**的缺口（断链、弱引用）。原被误归为「不计入 #1–#10」，现单独成表。
>
> 子表 A 含**已测试锁定**（4 个 `expectedFailure`，分属 G-A1×2、问题 4×2）与**仅记录（无测试）**两类——两类都必须在清单里，否则读者会误以为「只修那 4 个 xFail 就够了」。合计 **11 项**代码 / 设计缺口。
