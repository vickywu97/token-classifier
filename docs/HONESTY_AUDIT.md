...
| 引用溯源完整性（6 条 source_url 逐条抓取） | ⚠️ **3/6 有缺口** | HK-001/003 → `Virtual-assets` 实测 **404 断链**；HK-002 → `Licensing` 通用页存活但**无法印证 AMLO/VATP**（无 VASP/AMLO/2023-06-01）；SG-001/002/003 → 存活且主题吻合（PSA/DPT、SFA/资本市场）。见观察 4 与 #D1–#D3。 |
...
| 用例 | 输入（逐字） | 香港 SFC | 新加坡 MAS |
|------|-------------|---------|-----------|
| 1（港证 / 新非证） | `本代币代表公司发行的股份与债券，持有人享有 profit rights 分配。` | **likely_security**（HK-SFC-001: 股份 / 债券 / profit rights） | **insufficient_info**（零命中） |
| 2（新 DPT / 港非证） | `本代币可用于支付商品与服务，作为交易媒介与价值储存，支持跨境转账。` | **utility_token**（HK-SFC-003: 支付商品） | **payment_token**（SG-MAS-001: 支付 / 交易媒介 / 价值储存 / 转账） |
| 3（两者皆证） | `本代币为股权凭证，可在交易所交易，持有人享有投票权与分红。` | **likely_security**（HK-SFC-001: 股权 / 分红 / 投票权；HK-SFC-002: 交易所 [regime]） | **likely_security**（SG-MAS-002: 股权 / 投票权） |
| 4（两者皆非） | `本白皮书描述一个去中心化治理协议，无代币发行计划。` | **insufficient_info**（零命中） | **insufficient_info**（零命中） |
...
| 结果层 | ⚠️ **部分验证通过** | 仅 4 个手工用例；用例 1 初版因子串碰撞失败（已用 `profit rights` 修复） |
...
> - **子表 A — 代码 / 设计缺口（#1–#10）**：影响工具**逻辑 / 词表 / 判定 rubric** 的缺口（含需改代码与仅补注释两类），与「港新判定独立性」属于不同维度。
> - **子表 B — 数据质量缺口（#D1–#D3）**：影响**数据可信度 / 引用溯源**的缺口（断链、弱引用）。原被误归为「不计入 #1–#10」，现单独成表。
>
> 子表 A 含**已测试锁定**（4 个 `expectedFailure`，分属 G-A1×2、问题 4×2）与**仅记录（无测试）**两类——两类都必须在清单里，否则读者会误以为「只修那 4 个 xFail 就够了」。合计 **10 项**代码 / 设计缺口。
...
|---|------|------|---------|--------|---------|------|
| D1 | HK-SFC-002 | `source_url` 为 SFC「Licensing」通用落地页，**存活但无法印证 AMLO/VATP 定性**（无 VASP/AMLO/2023-06-01 字样） | WebFetch 抓取该 URL（2026-09-20）→ 仅述 SFO 中介人发牌职能 | 中（弱引用，非断链） | 替换为具体 VATP 发牌通函 URL（如 `apps.sfc.hk/edistributionWeb/gateway/TC/circular/doc?refNo=23EC28`）；条目加「引用待核实」标记 | 独立 |
| D2 | HK-SFC-001 | `source_url`（`.../Virtual-assets`）**实测 404 断链**（SFC 已迁移新主页，旧 URL 失效） | WebFetch 抓取（2026-09-20）→ 404，提示重定向至新站 | 中高（断链，完全不可溯源） | 更新为新站等效页或具体通函（条目 notes 已引「SFC 2019 声明 / 后续通函」，应取其真实 URL） | 独立 |
| D3 | HK-SFC-003 | `source_url` 与 001 **同一条死链**（`.../Virtual-assets`）→ 404 | WebFetch 抓取（2026-09-20）→ 404 | 中高（断链） | 同 D2，更新为可用 URL | 独立 |

> 子表 B 性质均为**数据质量 / 引用溯源缺陷，非法律错误、非代码缺陷**（HK-002 的 AMLO 定性、HK-001/003 的 SFO/utility 定性本身正确，问题在 `source_url` 不可达或不可印证）。与子表 A 的代码 / 设计缺口分开管理，但同样须在 README「已知限制」节披露。
